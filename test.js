...
test("One euro should be 1.206 dollars", function(){
    ...
    const { fromEuroToDollar } = require('./currency.js')
...
test("Dollar To Yen", function(){
    //import the function from app.js
    const { fromDollarToYen } = require('./currency.js')

    // use the function like its suppoed to be used
    const yen = fromDollarToYen(20)

    // if 1 euro are 1.206 dollars, then 3.5 euros should be (3.5 * 1.2)
    const expected = 2131.67; 
...
test("Yen To Pound", function(){
    const { fromYenToPound } = require('./currency.js')
    const pound = fromYenToPound(5000)
    const expected = 31.27; 
